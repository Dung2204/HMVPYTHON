...
def run_LinearRegression_app():
    ...
    with tab_log_info:
        ...
        try:
            ...
            if runs:
                run_options = {run.info.run_id: run.data.tags.get('mlflow.runName', 'Unnamed') for run in runs}
                selected_run_id_for_rename = st.selectbox("Chọn Run để đổi tên:", 
                                                          options=list(run_options.keys()), 
                                                          format_func=lambda x: run_options[x],
                                                          key="rename_select")
                new_run_name = st.text_input("Nhập tên mới cho Run:", 
                                             value=run_options[selected_run_id_for_rename])
                if st.button("Cập nhật tên Run"):
                    if new_run_name.strip():
                        client.set_tag(selected_run_id_for_rename, "mlflow.runName", new_run_name.strip())
                        # Cập nhật run_id trong st.session_state nếu trùng với run đang hiển thị
                        if 'reduce_results' in st.session_state and st.session_state['reduce_results'].get('run_id') == selected_run_id_for_rename:
                            st.session_state['reduce_results']['run_id'] = selected_run_id_for_rename
                        st.success(f"Đã cập nhật tên Run thành: {new_run_name.strip()}")
                        st.rerun()  # Sửa: Thay experimental_rerun bằng rerun
                    else:
            ...
            if runs:
                ...
                if st.button("Xóa Run", key="delete_run"):
                    ...
                    # Xóa dữ liệu trong st.session_state nếu run bị xóa là run hiện tại
                    if 'reduce_results' in st.session_state and st.session_state['reduce_results'].get('run_id') == selected_run_id_to_delete:
                        del st.session_state['reduce_results']
                        del st.session_state['X_reduced']
                        del st.session_state['reduce_model']
            ...
            if runs:
                ...
                st.info("Chưa có Run nào được log. Vui lòng thực hiện giảm chiều trước.")
